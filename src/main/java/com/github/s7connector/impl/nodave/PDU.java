...
public final class PDU {
    ...
    public void addBitVarToWriteRequest(final PlcArea area, final int DBnum, final int start, final int byteCount,
                                        final byte[] buffer) {
        ...
        if ((area == PlcArea.TIMER) || (area == PlcArea.COUNTER) || (area == PlcArea.TIMER200)
                || (area == PlcArea.COUNTER200)) {
            pa[3] = (byte) area.getCode();
            pa[4] = (byte) (((byteCount + 1) / 2) / 0x100);
            pa[5] = (byte) (((byteCount + 1) / 2) & 0xff);
        } else if ((area == PlcArea.ANALOGINPUTS200) || (area == PlcArea.ANALOGOUTPUTS200)) {
            pa[3] = 4;
            pa[4] = (byte) (((byteCount + 1) / 2) / 0x100);
            pa[5] = (byte) (((byteCount + 1) / 2) & 0xff);
        } else {
    ...
    public int addVarToReadRequest(final PlcArea area, final int DBnum, int start, final int len) {
        ...
        if ((area == PlcArea.ANALOGINPUTS200) || (area == PlcArea.ANALOGOUTPUTS200)) {
            pa[3] = 4;
            start *= 8; /* bits */
        } else if ((area == PlcArea.TIMER) || (area == PlcArea.COUNTER) || (area == PlcArea.TIMER200)
                || (area == PlcArea.COUNTER200)) {
            pa[3] = (byte) area.getCode();
        } else {
            start *= 8; /* bits */
    ...
    public void addVarToWriteRequest(final PlcArea area, final int DBnum, int start, final int byteCount,
                                     final byte[] buffer) {
     ...
     */
    public int getFunc() {
        return Nodave.USByte(this.mem, this.param);
    }
