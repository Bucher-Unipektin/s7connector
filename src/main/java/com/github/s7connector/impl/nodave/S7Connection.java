...
public abstract class S7Connection {
    ...
    public ResultSet execReadRequest(final PDU p) throws IOException {
        ...
        if (p2.mem[p2.param] == PDU.FUNC_READ) {
    ...
    public int readBytes(final PlcArea area, final int DBnum, final int start, final int len, final byte[] buffer) throws IOException {
        int res = 0;
        try {
    ...
    public int useResult(final ResultSet rs, final int number) {
        LOG.error("rs.getNumResults: " + rs.getNumResults() + " number: " + number);
     ...
     */
    public int writeBytes(final PlcArea area, final int DBnum, final int start, final int len, final byte[] buffer) throws IOException {
        int errorState = 0;
        this.semaphore.release();
        final PDU p1 = new PDU(this.msgOut, this.PDUstartOut);
